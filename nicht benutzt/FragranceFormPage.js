...
import { createFragance } from '../api';
...
const FragranceFormPage = () => {
  ...
  useEffect(() => {
    const fetchFragrance = async () => {
      ...
      try {
        setLoading(true);
        const data = await fragranceAPI.getFragranceById(id);
        setFormData(data);
        setError('');
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditMode) {
        await fragranceAPI.updateFragrance(id, formData);
      } else {
        await fragranceAPI.createFragrance(formData);
      }
